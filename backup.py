...
@app.route("/api/<country>/<state>")
def patent(country, state, year, cla):
      print state
      import json
      #conn = sqlite3.connect('/data/patentdata/LATEST/invpat.sqlite3')
      conn = sqlite3.connect('/Users/kevshin2/Desktop/location.sqlite3')
      #conn = sqlite3.connect('/Users/kevshin2/Documents/Fung_mapper/Patent_mapper/invpat.sqlite3')
      c = conn.cursor()
      if state == "empty":
          print "not here"
          c.execute('''
            select Patent, Longitude, Latitude, State, City, Country from location where Country = "{country}" limit 35000;
          '''.format(country=country))
          #nselect Patent, Longitude, Latitude, Lastname, Firstname, Assignee, AppYear, Gyear, State, City, Country from invpat where Country = "{country}" AND AppYear = {year} limit 35000;
      '''
      elif state == "empty" and cla != "empty":
          print "here"
          c.execute('''
           # select Patent, Longitude, Latitude, State, City, Country from location where Country = "{country}" AND AppYear = {year} AND Class like "{cla}/%" limit 35000;
      ...
      '''
          ...
          print "here2"
          c.execute('''
            select Patent, Longitude, Latitude, State, City, Country from location where Country = "{country}" AND State = "{state}" And Class like "{cla}/%" limit 35000;;
          '''.format(state=state, country=country))
      ...
      '''
          ...
          print "here3"
          c.execute('''
            #select Patent, Longitude, Latitude, State, City, Country from (select * from (select * from location where State = "{state}") where AppYear = {year});
...
def parse(filename):
  ...
  for line in f:
    ...
    line = int(line.strip())
    c.execute('''
      select Patent, Longitude, Latitude, State, City, Country from location where Patent = "{pat}";
      '''.format(pat=line))
